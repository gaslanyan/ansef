        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">Review and submit scores
                    <input style="margin-left:40px;" type="button" class="display btn btn-primary float-right"
                        onclick="open_container()" value="View refereeing guidelines">
                ...
                <div class="card-body" style="overflow:auto;">
                    <div class="row">
                        <a href="{{action('Referee\ReportController@show', $report->id)}}"
                            class="view float-left myButton" title="View"><i class="fa fa-eye"
                                style="margin-right:5px;"></i>View proposal
                        </a>
                        <a href="{{action('Referee\ReportController@generatePDF', $report->id)}}"
                            class="download float-left myButton" title="Download Report"><i class="fa fa-download"
                                style="margin-right:5px;"></i>Download proposal
                        </a>
                        <a style="margin-left:20px;"
                            href="{{action('Referee\SendEmailController@showEmail', $report->id)}}"
                            class="email float-left myButton" title="Send email"><i class="fa fa-envelope-open"
                                style="margin-right:5px;"></i>Contact program officer
                        </a>
                    </div><br /><br />
                    ...
                    <form method="post" action="{{ action('Referee\ReportController@update', $report->id) }}"
                        ...
                        <div class="form-group col-lg-12">
                            <p for="title">
                                <h5><b>{{getProposalTag($report->proposal->id)}}</b></h5>{{$report->proposal->title}}
                            </p>
                        </div>
                            ...
                            <div class="row">
                                <div class="form-group col-lg-12">
                                    ...
                                    <p style="color:#999;">Please provide a few words summarizing your assessment.
                                        The comments will be shared with the Principal Investigator of the proposal in
                                        an effort to help him or her
                                ...
                                <div class="form-group col-lg-12">
                                    ...
                                    <p style="color:#999;">You can provide the ANSEF Research Board with private
                                        comments about the proposal.
                                        These comments will NOT be shared with any members of the proposal and will be
                                        treated
                                        confidentially.
                                    </p>
                        ...
                        <div class="form-group col-lg-6">
                            ...
                            <div>
                                ...
                                <div class="row col-12">
                                    ...
                                    <select max="{{$scoreType->max}}" weight="{{$scoreType->weight}}"
                                        class="form-control col-6 scoremenus" name="name[{{$scoreType->id}}]"
                                        id="{{$scoreType->id}}">
                                        @foreach($vals as $val)
                                        <option value="{{$val}}"
                                            {{$val == $scores[$scoreType->id]->value ? 'selected' : ''}}>
                                            {{$val}}{{$val == 0 ? ' (Poor)' : ''}}{{$val == $scoreType->max ? ' (Outstanding)' : ''}}
                                        </option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div class="form-group col-lg-12">
                            ...
                            <button style="margin-left:10px;" type="submit" name="submitaction" value="in-progress"
                                class="btn btn-primary float-right">
                                Save but do not submit
                            </button>
                            <button
                                onclick="return confirm('Are you sure you want to submit your report? No further changes will be possible.');"
                                style="margin-left:10px;" type="submit" name="submitaction" value="complete"
