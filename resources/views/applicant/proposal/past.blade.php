                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="{{action('Applicant\FileUploadController@reportfile', $ap['id'])}}"
                                        title="View">
                                        @if($firstreports[$ap['id']])
                                        <span class="fas fa-file-upload myButton">&nbsp;First Report</span>
                                        @else
                                        <span class="fas fa-file-upload myButton" style="color:#e00;">&nbsp;First
                                            Report</span>
                                        @endif
                                    </a>
                                    @endif
                                    @if($ap->competition->second_report >= date('Y-m-d') &&
                                    $ap->competition->first_report < date('Y-m-d')) <a
                                        href="{{action('Applicant\FileUploadController@reportfile', $ap['id'])}}"
                                        title="View">
                                        @if($secondreports[$ap['id']])
                                        <span class="fas fa-file-upload myButton">&nbsp;Second Report</span>
                                        @else
                                        <span class="fas fa-file-upload myButton" style="color:#e00;">&nbsp;Second
                                            Report</span>
                                        @endif
                                        </a>
                                        @endif
                            ...
                            <tr>
                                ...
                                <td data-order="{{$ap['state']}}" data-search="{{$ap['state']}}">
                                    ...
                                    @elseif($ap->state == 'finalist' && $ap->competition->results_date < date('Y-m-d'))
                                        <b>Finalist</b>
                                        @elseif($ap->state == 'finalist' && $ap->competition->results_date >=
                                        date('Y-m-d'))
                                        Reviews complete
                                        @else
                                        @endif
