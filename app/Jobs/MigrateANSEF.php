...
class MigrateANSEF implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        // Add user and person for admin
            ...
            $psc = Category::where('abbreviation', '=', $subcategories[$proposal->subcategory_id]->label)->first();
            $pssc = Category::where('abbreviation', '=', $subcategories[$proposal->secondary_subcategory_id]->label)->first();
            $pc = Category::find($psc->parent_id);
            $psc = Category::find($pssc->parent_id);
            $cat = [];
            $cat["parent"] = $pc->id;
            $cat['sub'] = $psc->id;
            $cat["sec_parent"] = $psc->id;
            $cat["sec_sub"] = $pssc->id;
