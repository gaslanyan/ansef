    ...
    public function activateUser(string $useremail, string $activationCode)
    {
        try {
            ...
            if($user->state === "waiting") {
                $role = Role::where('id', '=', $user->role_id)->first();
                return redirect()->route('login.' . $role->name)->with('status',"Your account is awaiting activation by the portal administrator. Try again later, or contact help@ansef.org.");
            }
